...
tar_list="$(tar -tf "$bundle" 2>/dev/null)" || { log "archive validation failed: tar -tf could not list $bundle (corrupt/non-archive?); aborting before any destructive action"; exit 50; }
