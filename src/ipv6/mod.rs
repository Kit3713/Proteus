...
/// Plain-Rust SHA-256. Inlined here so we don't pull in `sha2`/`digest`
/// (about 250 KB combined) just for the drop-in header.
...
fn sha256(data: &[u8]) -> [u8; 32] {
    // FIPS 180-4 §6.2.2. Standard 32-bit reference implementation; not the
    // fastest, but it only runs once per `apply`.
    const K: [u32; 64] = [
        0x428a_2f98,
        0x7137_4491,
        0xb5c0_fbcf,
    ...
    for chunk in padded.chunks_exact(64) {
        let mut w = [0u32; 64];
        for (i, slice) in chunk.chunks_exact(4).enumerate() {
            w[i] = u32::from_be_bytes([slice[0], slice[1], slice[2], slice[3]]);
        }
        for i in 16..64 {
