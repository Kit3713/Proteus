...
/// `src/commands/events.rs` which is a Group A file in the Wave 3
/// partition. Group A wires `tokio::signal::unix::SignalKind::hangup`
/// into the daemon and calls `swap()`. This module owns the
/// captive-portal-side primitive so Group A doesn't have to invent a
/// concurrency story for one config block.
///
