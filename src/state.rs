...
// Roadmap 1.1.1: the pure leaf DTOs of the State tree moved to
// `proteus-types`. Re-exported here so every `crate::state::Originals`,
// `crate::state::InterfaceRecord`, ... path in the binary keeps resolving
// and the `--json` / on-disk shape is byte-identical. `State` itself stays
// below — its `load`/`save` impl is bound to `commands::write_atomic` and
// `KillSwitchState`, which don't belong in a pure-DTO crate.
pub use proteus_types::state::{
    ConnectionOriginals, ConnectionRecord, DhcpSettingsSnapshot, HostnameOriginals,
    InterfaceRecord, Ipv6Originals, ManagedState, Originals, PerSsidStateSeed, PortalCheckRecord,
    RfOriginals,
};
