...
#   6. binary size <= 3,000,000 bytes (hard project invariant)
...
size_check() {
    ...
    printf 'PROTEUS_BINARY_SIZE_LIMIT_BYTES=3000000\n'
    if [ "$size" -gt 3000000 ]; then
        printf 'FAIL: stripped proteus binary is %s bytes, exceeds 3,000,000 byte cap\n' \
            "$size" >&2
        return 1
    fi
    printf 'OK: stripped proteus binary is %s bytes (under 3,000,000 byte cap)\n' \
        "$size"
}
run_step "binary size <= 3 MB" size_check
