...
> v0.3 cycle "Reach + Persona" (see [`ROADMAP-v0.3.md`](ROADMAP-v0.3.md))
> shipped the `NetworkBackend` abstraction, persona / per-SSID modes, RF
> finishing, distro reach, and ergonomics on top of Phase G. The active
> cycle is v0.4.x "Hardening Across Streams" — ten parallel workstreams
> against the issue inventory in [`ISSUES.md`](ISSUES.md); no new
> features. See [`ROADMAP.md`](ROADMAP.md). The text below is preserved
> as the design rationale; cross-check claims like "deferred to v2"
> against the roadmap and CHANGELOG before quoting.
