...
- **Custom user profiles can be either kind.** A user can author a custom randomizer recipe (different OUI pools, different cadences than the built-in slider) OR a custom stealth persona (specific device cover) — and switch between them at will. Built-in catalogue and user catalogue are both read by the same loader.
